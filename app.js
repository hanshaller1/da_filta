...
const state = { inputGain:0, resonance:0, dryWet:50, spread:0, volume:-6, bandGains:Array(10).fill(0), feedbackBand:Array(10).fill(false), modulated:Array(10).fill(false), feedbackAll:false };
const bands = document.querySelector('.bands');
bands.innerHTML = BAND_FREQUENCIES.map((frequency,index) => `<article class="band-card"><div class="band-title">BAND ${index+1}</div><div class="band-actions"><button class="band-action" type="button" data-feedback-band="${index}">FB</button><button class="band-action" type="button" data-mod-band="${index}">MOD</button></div><div class="fader-wrap"><span class="fader-label positive">+12</span><div class="fader-track"><input class="band-fader" type="range" min="-100" max="100" value="0" data-band="${index}" aria-label="${frequency} Fader"></div><span class="fader-label negative">-12</span></div><div class="band-value">${frequency}</div></article>`).join('');
...
const updateAnalyzerBand = (index) => {
  ...
  const normalized = (state.bandGains[index] - min) / (max - min);
...
const setBandGain = (index, value) => {
  ...
  state.bandGains[index] = nextValue;
  slider.value = String(nextValue);
  updateAnalyzerBand(index);
};
faders.forEach((slider,index) => {
  slider.addEventListener('input', () => setBandGain(index, slider.value));
  setBandGain(index, 0);
});
...
document.querySelectorAll('[data-feedback-band]').forEach(button => button.addEventListener('click', () => { const index=Number(button.dataset.feedbackBand); state.feedbackBand[index]=!state.feedbackBand[index]; button.classList.toggle('active',state.feedbackBand[index]); button.setAttribute('aria-pressed',String(state.feedbackBand[index])); }));
...
fbAllButton.addEventListener('click', () => { state.feedbackAll=!state.feedbackAll; fbAllButton.classList.toggle('active',state.feedbackAll); fbAllButton.textContent=state.feedbackAll?'ON':'OFF'; fbAllButton.setAttribute('aria-pressed',String(state.feedbackAll)); });
...
document.addEventListener('keydown', event => {
  ...
  if (bandIndex !== -1) {
    if (event.shiftKey) document.querySelector(`[data-mod-band="${bandIndex}"]`).click();
    else document.querySelector(`[data-feedback-band="${bandIndex}"]`).click();
    event.preventDefault();
    return;
  }
  const upIndex = FADER_UP_CODES.indexOf(event.code);
  if (upIndex !== -1) {
    const slider = faders[upIndex];
    setBandGain(upIndex, Number(slider.value) + (Number(slider.max) - Number(slider.min)) * 0.05);
    event.preventDefault();
    return;
  }
  const downIndex = FADER_DOWN_CODES.indexOf(event.code);
  if (downIndex !== -1) {
    const slider = faders[downIndex];
    setBandGain(downIndex, Number(slider.value) - (Number(slider.max) - Number(slider.min)) * 0.05);
    event.preventDefault();
  }
});
