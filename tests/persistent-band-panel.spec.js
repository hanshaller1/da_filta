...
test('desktop keeps permanent band controls aligned and stable across every workspace', async ({ page }) => {
  ...
  expect(panel.left).toBe(152);
  expect(panel.right).toBe(1573);
  expect(panel.width).toBe(1421);
  expect(Math.abs(channelPanel.left - modeTabs.left)).toBeLessThan(1);
  ...
  expect(workspace.height).toBe(450);
  expect(graph.height).toBeGreaterThan(330);
  expect(firstCard.height).toBe(310);
