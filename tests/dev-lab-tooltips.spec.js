...
test('all DEV/LAB properties expose complete hover and keyboard help', async ({ page }) => {
  ...
  expect(await controls.count()).toBe(20);
  ...
  for (let index = 0; index < await controls.count(); index += 1) {
    ...
    await expect(tooltip.locator('li')).toHaveCount(optionCount);
  ...
  await controls.first().locator('select').focus();
  await expect(tooltip).toBeVisible();
  await controls.last().locator('select').focus();
  await expect(tooltip).toBeVisible();
