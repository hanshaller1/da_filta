...
const normalControls = {
  ...
  volume: '[data-control="volume"]', band: '.band-fader[data-band="1"]:not([data-channel])'
};
const openGroup = async (page, group) => {
  const toggle = page.locator(`[data-dev-lab-group="${group}"] .dev-lab-collapse-toggle`);
...
test.describe('DEV/LAB snapshots', () => {
  test('save contains only the explicit DEV/LAB configuration', async ({ page }) => {
    ...
    await page.locator('[data-control="spread"]').fill('-0.3');
    await page.locator('[data-control="volume"]').fill('-12');
    await page.locator('.band-fader[data-band="0"]:not([data-channel])').fill('75');
    ...
    await setCompatibilityCurve(page, 'quadratic');
  ...
  test('load restores DEV/LAB values while preserving normal controls and feedback state', async ({ page }) => {
    ...
    await page.locator('[data-wet-model]').selectOption('reference-delta');
    await setCompatibilityCurve(page, 'smoothstep');
    ...
    await page.locator(normalControls.spread).fill('-0.3');
