...
const expectedGroups = {
  ...
  output: ['SOFT PROTECTION', 'THRESHOLD', 'SOFTNESS'],
...
test('DEV-LAB exposes complete click-open group help, including OUTPUT', async ({ page }) => {
  await page.setViewportSize({ width: 1440, height: 900 });
  await page.goto('/', { waitUntil: 'networkidle' });

  const tooltip = page.locator('#dev-lab-tooltip');
  await expect(tooltip).toBeHidden();
  await expect(page.locator('.dev-lab-group-header .dev-lab-info-button')).toHaveCount(8);

  await page.locator('[data-dev-lab-group="input"] .dev-lab-collapse-toggle').click();
