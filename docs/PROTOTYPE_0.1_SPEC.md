...
- positiver TPT-Resonance-Audition-Gain: `0.10` (Prototype-Hörtestwert); er befindet sich in manueller Hörkalibrierung. Der temporäre `DEV RES AUD`-Selector bietet `0.10`, `0.20`, `0.30`, `0.40`, `0.60`, `0.80`, `1.00`, `1.50`, `2.00` und `4.00`; Werte über Unity sind im DEV/LAB ausdrücklich erlaubt. Default nach Reload bleibt `0.10`, ein finaler Classic-Wert ist noch nicht festgelegt.
- positiver TPT-Resonance-Drive: temporärer `DEV RES DRIVE`-Selector mit `1`, `2`, `4`, `8`, `16`, `24` und `32`; Default nach Reload ist `1`. Er steuert ausschließlich die interne nichtlineare State-Feedback-Saturation und ist kein finales UI-Feature oder Erica-Hardwarewert.
- positiver TPT-Resonance-Damping-Floor: temporärer `DEV RES FLOOR`-Selector mit `0.10`, `0.05`, `0.02`, `0.00`, `-0.02`, `-0.05` und `-0.10`; Default nach Reload ist `0.10`. Er bestimmt nur die Restdämpfung des lokalen positiven nichtlinearen Resonators bei voller Resonance. Werte `<= 0` dienen ausschließlich der Grenz- und Selbstoszillationsanalyse; sie sind weder finale Classic-Werte noch behauptete Erica-Hardwarewerte.
- DEV/LAB Phase A ergänzt ausschließlich experimentelle Hörvergleichsoptionen für die positive lokale TPT-Resonance: `DEV RES OUTPUT` mit `CURRENT RESIDUAL`, `NONLINEAR - BASE` und `FULL NONLINEAR`; `DEV RES LATENCY` mit `CURRENT` und `MATCHED`; sowie `DEV RES CURVE` mit `CURRENT`, `EARLY` und `AGGRESSIVE`. `CURRENT / CURRENT / CURRENT` bleibt der bisherige Produktionsstand. Die Varianten sind keine finale Architektur- oder Klangentscheidung.
- DEV/LAB Phase B1 ergänzt experimentelle Hörvergleichsoptionen: `DEV REFERENCE` mit `100`, `75`, `50`, `25` und `0 % / BANDS ONLY`; `DEV RES ENGINE` mit `TPT` und `PHASE 2`; `DEV BAND BOOST` mit `+12`, `+18`, `+24 dB`; sowie `DEV BAND CUT` mit `-12`, `-24`, `-36`, `-48`, `-60 dB`. Die Reload-Defaults `100 % / TPT / +12 / -12 dB` entsprechen dem bisherigen Stand. Keine Variante ist final kalibriert.
- DEV/LAB Phase B2 ergänzt experimentelle Reverse-Engineering-Hörvergleichsoptionen: `DEV FB TOPOLOGY` mit `ISOLATED TPT` und `COMMON BUS`, `DEV FB TAP` mit `PRE GAIN` und `POST GAIN`, sowie `DEV WET MODEL` mit `REFERENCE + DELTA` und `FILTERBANK SUM`. Sie betreffen nur positive lokale Resonance; FB ALL und negative Resonance bleiben Legacy. Keine Option behauptet Kenntnisse der Erica-internen Schaltung oder ist als finale Architektur festgelegt.
- DEV/LAB Phase B3 ergänzt nur für `COMMON BUS` die experimentellen Parameter `DEV FB SAT` (`CURRENT` / `CONSTANT CEILING`), `DEV FB DRIVE` (`0.5`, `1`, `2`, `4`, `8`, `16`) und `DEV FB CEILING` (`0.25`, `0.50`, `1`, `2`, `4`). `CONSTANT CEILING` verwendet `ceiling * tanh((drive * x) / ceiling)`. Diese Parameter sind keine finale Erica-Emulation.
- DEV/LAB Phase B4 ergänzt `DEV FB ALL ENGINE` (`LEGACY` / `COMMON BUS`) und `DEV FB ALL SOURCE` (`PRE GAIN SUM` / `POST GAIN SUM`). Im positiven Common-Bus-Modus besitzt MAIN einen eigenen saturierten, ein Sample verzögerten Return aus der internen Base-Filterbank-Summe. Lokaler Common Return und MAIN-Return werden getrennt gesättigt und erst am gemeinsamen Base-Filterbank-Eingang addiert. Negative Resonance bleibt Legacy. Diese Reverse-Engineering-Optionen behaupten keine Kenntnis der internen Erica-Schaltung.
- `DEV FB ALL LEVEL` ergänzt für COMMON-BUS-MAIN eine experimentelle feste Summenskalierung vor Resonance-Gain und Saturation: `RAW` (`1.000`), `1 / SQRT(2)` (ca. `0.707`), `1 / 2` (`0.500`), `1 / SQRT(10)` (ca. `0.316`), `1 / 10` (`0.100`), `1 / 20` (`0.050`), `1 / 40` (`0.025`) und `1 / 80` (`0.0125`). Reload-Default ist `RAW`; Legacy FB ALL und der lokale Common Bus bleiben davon unbeeinflusst. Dies ist eine experimentelle COMMON-BUS-MAIN-Kalibrierung; keine Variante ist als finale Lösung deklariert.
- DEV/LAB-Controls werden standardmäßig angezeigt und können über den Header-Button `DEV/LAB` ein- und ausgeblendet werden. Das Panel erscheint rechts neben dem Hauptinterface ohne internen Scrollbereich; Ein-/Ausblenden erhält Werte und Funktionalität unverändert.
- Jedes DEV/LAB-Property besitzt kontextbezogene Hilfe mit Property, Scope und vollständiger Value-Erklärung; die Tooltips funktionieren per Hover und Keyboard-Focus und ändern keine DSP-Funktion.
- DEV/LAB Phase C1 bietet `DEV INPUT STAGE` mit `LINEAR` (Reload-Default), `CLEAN`, `WARM`, `CRUNCH` und `AGGRESSIVE`. Der bestehende Input Gain von `0 … +24 dB` bleibt eine reale lineare Verstärkung vor der gewählten festen, experimentellen Drive-Kennlinie und steuert deren Ansteuerung kontinuierlich. Die Character-Auswahl morpht nicht mit dem Gain. `PREAMP` ist kein gültiger Stage-Wert mehr; alte oder ungültige Werte fallen kontrolliert auf `LINEAR` zurück. Es gibt keine Loudness-Kompensation; Volume bleibt der separate Ausgangsregler. Die Drive-Typen sind Webapp-Charaktere und keine behauptete Erica-Schaltungsnachbildung.
