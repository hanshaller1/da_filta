...
- DEV/LAB Phase C1 ergänzt `DEV INPUT STAGE` mit `LINEAR` (Reload-Default) und `PREAMP`. Der bestehende Input Gain von `0 … +24 dB` bleibt eine reale lineare Verstärkung; `PREAMP` folgt danach vor der Dry/Wet-Verzweigung mit einer kontinuierlichen, symmetrischen Soft-Knee-Färbung von clean über warm und crunch bis aggressive. Es gibt keine Loudness-Kompensation; Volume bleibt der separate Ausgangsregler. Dies ist ein experimenteller Webapp-Prototyp, keine behauptete Erica-Schaltungsnachbildung.
